...
const customers: Module<State, Rootstate> = {
  ...
  mutations: {
    ...
    [MutationTypes.RemoveCustomer](state: State, id: number) {
      state.customerRequestStatus.DeleteCustomer = status.success;
    },
  },
  ...
  actions: {
    ...
    async getCustomer({ commit }: { commit: Commit }, id: customers) {
      try {
        MutationTypes.CustomerLoading;
        const { data } = await axios.get("http://slimapp/api/customers/" + id);
        commit(MutationTypes.SetCustomers, data);
      } catch (e) {
        commit(MutationTypes.CustomerError, "error");
        console.log("error: ", e);
    ...
    async DeleteCustomer({ commit }: { commit: Commit }, id: customers) {
      try {
        MutationTypes.CustomerLoading;
        await axios.delete(`http://slimapp/api/customer/delete/${id}`);
        commit(MutationTypes.RemoveCustomer, id);
      } catch (e) {
        commit(MutationTypes.CustomerError, "error");
        console.log("error: ", e);
        //error
